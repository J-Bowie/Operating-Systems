...
extern Scheduler* SYSTEM_SCHEDULER; // Call SYSTEM_SCHEDULER with extern global function
...
static void thread_shutdown() {
    ...
    SYSTEM_SCHEDULER->terminate(Thread::CurrentThread()); // First step is to terminate the function
    delete current_thread; // Next we delete the current running thread to get it off memory (since terminate only removes from queue)
    SYSTEM_SCHEDULER->yield(); // Yield the thread to wait for next call
    // Was told during help session to add assertion here to stop ERROR <4> (EDITED: Removed due to assertion call by FUN2)
